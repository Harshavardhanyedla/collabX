...
export const fetchConnections = async (userId: string) => {
    ...
    // 1. Get all accepted connections where I am requester
    const q1 = query(
        connectionsRef,
        where('requesterId', '==', userId),
        where('status', '==', 'accepted')
    ...
    // 2. Get all accepted connections where I am recipient
    const q2 = query(
        connectionsRef,
        where('recipientId', '==', userId),
        where('status', '==', 'accepted')
    ...
    sent.docs.forEach(doc => partnerIds.add(doc.data().recipientId));
    received.docs.forEach(doc => partnerIds.add(doc.data().requesterId));

    if (partnerIds.size === 0) return [];

    // 4. Fetch user profiles
    // Firestore 'in' query supports max 10 items. For scalability, we should batch or fetch individually.
    // For simplicity here, we'll fetch individually in parallel (fine for <100 connections in this demo).

    const { getUserProfile } = await import('./profiles');
    const profiles = await Promise.all(
        Array.from(partnerIds).map(id => getUserProfile(id))
    );
